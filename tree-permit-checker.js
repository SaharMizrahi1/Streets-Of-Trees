...
const serviceAccount = JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT); // Parse service account from .env
...
const SMOOVE_API_KEY = process.env.SMOOVE_API_KEY; // 🔑 Replace with your actual Smoove API key
...
async function findSubscribedUsers(settlement) {
  ...
  try {
    const notificationsSnapshot = await db.collection('notifications')
      ...
      const users = notificationsSnapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: data.userId, // Using userId from notifications
          email: data.userEmail, // Directly using userEmail from notifications
          name: data.userEmail // Use email as name (or adjust if you add name field in the future)
        };
      });
  ...
  //   const userIds = notificationsSnapshot.docs.map(doc => doc.data().userId);

  //   // Fetch user emails/names from 'users' collection
  //   const userFetches = userIds.map(userId => db.collection('users').doc(userId).get());
...
module.exports = { checkForNewPermits }; // Optional export
